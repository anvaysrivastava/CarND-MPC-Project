...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    // Credit is given to course material. I have mostly copied from MPC solution showcased in Lesson 19: 9. Solution: Mind the line
    ...
    for (int i = 0; i < N; i++) {
      // Penalty for error from centre
      fg[0] += 1000*CppAD::pow(vars[cte_start + i], 2);
      // Penalty for orientation diff
      fg[0] += 1000*CppAD::pow(vars[epsi_start + i], 2);
      // Ensuring we don't get stuck in middle
    ...
    for (int i = 0; i < N - 2; i++) {
      // Penalty for smoother change of seering
      fg[0] += 1000*CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
      // Penalty for smoother change of acceleration
